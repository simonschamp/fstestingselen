...
router.put("/reorder", async (req, res) => {
    try {
        ...
        const updates = orderedIds.map((id, index) => Message.findByIdAndUpdate(id, { order: index }));
...
router.put("/:id", async (req, res) => {
    try {
        const { title, content } = req.body;
        const updated = await Message.findByIdAndUpdate(req.params.id, { title, content }, { new: true } // return updated document
        );
        if (!updated) {
